...
const Payment = () => {
    ...
    useEffect(() => {
        dispatch(FlightDetails("c198a007-2a08-45a9-9d41-e4d19a80dc27"))
        dispatch(PaymentCards())
        dispatch(BookingPaymentDetails("af06841c-11d2-4f62-8193-31b872705a26"))
    }, [])
    const flightDetails = useSelector((state) => state.FetchFlightDetails)

    const bookingPayement = useSelector((state) => state.FetchBookingPaymentDetails)
    const cards = useSelector((state) => state.FetchPaymentCards)
    const defaultCard = cards.data[0] ? cards.data[0].id : ''
    
    const bookingId = bookingPayement.data.bookingPaymentData ? bookingPayement.data.bookingPaymentData[0].id.toUpperCase() : ''
    const paymentId = bookingPayement.data.bookingPaymentData ? bookingPayement.data.bookingPaymentData[0].id_payments.toUpperCase() : ''
    const totalSeats = bookingPayement.data.bookingPaymentData ? bookingPayement.data.bookingPaymentData[0].total_seats : ''
    const totalLuggage = bookingPayement.data.bookingPaymentData ? bookingPayement.data.bookingPaymentData[0].total_luggage : ''
    const totalPrice = bookingPayement.data.bookingPaymentData ? bookingPayement.data.bookingPaymentData[0].total_price : ''
                ...
                <section className="payment-wrapper d-flex">
                    <div className="payment-left d-flex flex-fill flex-column w-75 h-100 bg-white rounded me-3 p-3">
                        <div className="font-18 text-center mb-3">
                            ...
                            {
                                passenger_list ? passenger_list.map((item, index) => {
                                    return (
                                        <div key={index} className='card d-flex flex-row p-3 mb-2'>
                                            <div className="col">
                                                ...
                                                <div className="font-16 mb-2">
                                                    Price : $ {flightDetails.data[0]? flightDetails.data[0].price : ''}
                                                </div> 
                                                <div className="font-16 mb-2">
                                            ...
                                            <div className="col">
                                                <div className="font-16 mb-2">
                                                    {flightDetails.data[0]? flightDetails.data[0].airline_name : ''} / {flightDetails.data[0]? flightDetails.data[0].flight_number : ''}
                                                </div>
                                                <div className="font-16 mb-2">
                                                    Price : $ {flightDetails.data[0]? flightDetails.data[0].price : ''}
                                                </div> 
                                            ...
                                            <div className="col">
                                                <div className="font-16 mb-2">
                                                    {flightDetails.data[0]? flightDetails.data[0].origi_city : ''} <img className='mx-3' src={greyAircraft} alt="" /> {flightDetails.data[0]? flightDetails.data[0].destination_city : ''}
                                                </div>
                                                <div className="font-16 mb-2">
                                                    Price : $ {flightDetails.data[0]? flightDetails.data[0].price : ''}
                                                </div> 
                                    ...
                                    <div className="card d-flex mb-3 p-3 w-100 ms-2 blue2 rounded">
                                        <div className="d-flex flex-column">
                                            ...
                                            <div className="d-flex justify-content-between align-items-center">
                                                ...
                                                <div className="d-flex">
                                                    {cards.data[0] ? cards.data[0].card_provider === 'VISA' ? (<img className='mx-3' src={visaCard} alt="" width={50}/>) : (<img className='mx-3' src={masterCard} alt="" width={50}/>) : 'null'}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <input type="radio" value={cards.data[0] ? cards.data[0].id : 'null'} checked={cards.data[0] ? selectedPaymentCard === cards.data[0].id : 'null'} onChange={handleOptionChange} />
                                    ...
                                    <div className="card d-flex mb-3 p-3 w-100 ms-2 blue2 rounded">
                                        <div className="d-flex flex-column">
                                            ...
                                            <div className="d-flex justify-content-between align-items-center">
                                                ...
                                                <div className="d-flex">
                                                    {cards.data[1] ? cards.data[1].card_provider === 'VISA' ? (<img className='mx-3' src={visaCard} alt="" />) : (<img className='mx-3' src={masterCard} alt="" width={50}/>) : 'null'}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <input type="radio" value={cards.data[1] ? cards.data[1].id : 'null'} checked={cards.data[1] ? selectedPaymentCard === cards.data[1].id : 'null'} onChange={handleOptionChange} />
                                    <span className="checkmark"></span>
                                </div>
                            </form>
                        </div>
                        ...
                        <div className="payment-details d-flex flex-fill flex-column p-3">
                            ...
                            <div className="d-flex justify-content-between mt-3">
                                <div className="font-12 mb-2">
                                    Seats : {totalSeats} Persons
                                </div>
                                <div className="font-12 mb-2">
                                    Luggage : {totalLuggage} kg
                                </div> 
                            </div>
                            <hr />
                            <div className="d-flex justify-content-between">
                            ...
                            <div className="d-flex p-5">
                                <div className="d-flex flex-fill justify-content-center">
                                    <Button className='btn-sign bg-primary rounded-3 p-2 text-white shadow' onClick={handleSubmitClick}>
                                        Pay
                                    </Button>
                                </div>
                            </div>
